...
//   loan_price: number;
//   admin_accept: "faild" | "accepted"; // Adding admin_accept field
// }
...
//                     <td className="py-10 px-10 xl:whitespace-nowrap">درخواست {row.requestNumber}</td>
...
//                             {row.guarantors.length > 0 ? row.guarantors.join(", ") : null}
//                           </span>
//                         </div>
//                         <div>
...
//                           <a
//                             className={`flex gap-3 items-center border-2 text-black font-bold rounded-md px-5 py-2 ${
//                               row.admin_accept === "faild" ? " border-[#4FD1C5] text-[#4FD1C5]" : "border-[#000000]"
//                             }`}
//                           >
//                             <Image
//                               src={row.admin_accept === "faild" ? AcceptIcon : RejectIcon}
//                               width={24}
//                               height={24}
//                               alt={row.admin_accept === "faild" ? "RejectIcon" : "AcceptIcon"}
//                             />
//                             {row.admin_accept === "faild" ? "تایید شده" : "رد شده"}
//                           </a>
//                         </div>
//                       </div>
//                     </td>
...
import React, { useState } from "react";
import Image from "next/image";
import RejectIcon from "@/app/assent/Img/adminPanel/RejectCross.svg";
import AcceptIcon from "@/app/assent/Img/adminPanel/AcceptTik.svg";
import Paginate from "./Paginate";
                    ...
                    <td colSpan={5} className="py-2 px-4 border-b border-opacity-80 border-[#0000001A]">
                      <div className="flex justify-between items-center">
                        ...
                        <div className="flex items-center gap-3">
                          <a
                            className={`flex gap-3 items-center border-2 text-black font-bold rounded-md px-5 py-2 ${
                              row.admin_accept === "faild" ? "border-[#000000]" : "border-[#4FD1C5] text-[#4FD1C5]"
                            }`}
                          >
                            <Image
                              src={row.admin_accept === "faild" ? RejectIcon : AcceptIcon}
                              width={24}
                              height={24}
                              alt={row.admin_accept === "faild" ? "RejectIcon" : "AcceptIcon"}
                            />
                            {row.admin_accept === "faild" ? "رد شده" : "تایید شده"}
                          </a>
                        </div>
                      </div>
                    </td>
