...
function SupportChatComponent() {
  const message = [{ id: 1, src: Image1, name: "امیر قنبری" }];
  const [messages, setMessages] = useState([
    {
      id: 1,
      text: "سلام من هرکاری میکنم نمیتونم رسید پرداختو بارگذاری کنم و خطا میده باید چیکار کنم؟؟؟",
      sender: "user",
    },
    {
      id: 2,
      text: "سلام، لطفا مطمئن شوید که فایل شما در قالب درست است.",
      sender: "admin",
    },
  ]);
  ...
  const handelSend = () => {
    if (newMessage.trim()) {
      setMessages([
        ...messages,
        { id: messages.length + 1, text: newMessage, sender: "admin" },
      ]);
      setNewMessage("");
    }
  };
      ...
      <div>
        ...
        <div className="bg-white w-[1057px] h-[771px] shadow-lg mt-5 px-14 py-2 pb-4  rounded-sm">
          {message.map((user, index) => (
            <div
              key={index}
              className={`flex w-[100%] mt-5 py-2 gap-4  items-start rounded-md ${
                index % 2 === 0
            ...
            >
              ...
              <div className="text-right">
                <p className="font-bold text-[#003B33]">{user.name}</p>
              </div>
            </div>
          ))}
              ...
              >
                <p
                  className={`p-3 rounded-lg inline-block max-w-4/5 ${
                    message.sender === "user" ? "bg-[#4FD1C50D]" : "bg-gray-100"
                  }`}
                >
                  {message.text}
                </p>
