...
const fetchChartData = async (): Promise<ChartData> => {
  ...
  const inventoryData = response.data.inventor;

  return {
    income: inventoryData.map((item: any) => item.income),
    outcome: inventoryData.map((item: any) => item.outcome),
  };
};
...
function ChartAdminDashboard() {
  ...
  const chartConfig = {
    ...
    datasets: [
      {
        label: 'درآمد',
        data: data?.income || [120, 90, 60, 30, 10, 0], // Fallback data if API fails
        borderColor: 'rgba(54, 162, 235, 1)',
        backgroundColor: 'rgba(54, 162, 235, 0.2)',
        fill: false,
        tension: 0.1,
      ...
      {
        label: 'خروجی',
        data: data?.outcome || [120, 90, 60, 30, 10, 0], // Fallback data if API fails
        borderColor: 'rgba(255, 99, 132, 1)',
        backgroundColor: 'rgba(255, 99, 132, 0.2)',
        fill: false,
        tension: 0.1,
